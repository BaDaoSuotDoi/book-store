...
// let isMoved = false;
// let listItemTouched: any;
// let index:any;
// let clientX:any;
// let clientY: any;

class DragList<Value = string> extends React.Component<IProps<Value>> {
  ...
  getChildren = () => {
    ...
    console.warn(
      'No items found in the DragList container. Did you forget to pass & spread the `props` param in renderList?'
    );
    return [];
  };
  ...
  onMouseOrTouchStart = (e: MouseEvent & TouchEvent) => {
    ...
    this.index = this.getTargetIndex(e as any);
    if (
      this.index === -1 ||
      // @ts-ignore
      (this.props.values[this.index] && this.props.values[this.index].disabled)
    ) {
    ...
    this.listItemTouched = this.getChildren()[this.index] as HTMLElement;
    const handle = this.listItemTouched.querySelector('[data-movable-handle]');
    if (handle && !handle.contains(e.target as any)) {
      return;
    }
    if (checkIfInteractive(e.target as HTMLElement, this.listItemTouched)) {
      return;
    }
    e.preventDefault();
    ...
    if (isTouch) {
      ...
      this.listItemTouched.style.touchAction = 'none';
    ...
    this.clientX = isTouch ? e.touches[0].clientX : e.clientX;
    this.clientY = isTouch ? e.touches[0].clientY : e.clientY;
    this.onStart(
      this.listItemTouched,
  ...
  onStart = (
    ...
    // this.state.itemDragged = index;
    // this.state.targetX=
    //     targetRect.left - parseInt(targetStyles['margin-left' as any], 10);
    // this.state.targetY= targetRect.top - parseInt(targetStyles['margin-top' as any], 10),
    // this.state.targetHeight= targetRect.height,
    // this.state.targetWidth= targetRect.width,
    // this.state.initialX= clientX,
    // this.state.initialY= clientY
    // this.setState({
    //   itemDragged: index,
    //   targetX:
    //     targetRect.left - parseInt(targetStyles['margin-left' as any], 10),
  ...
  onMouseMove = (e: MouseEvent) => {
    ...
    if(!this.isMoved){
            const targetRect = this.listItemTouched.getBoundingClientRect() as DOMRect;
            const targetStyles = window.getComputedStyle(this.listItemTouched);
            this.setState({
            itemDragged: this.index,
            targetX:
                targetRect.left - parseInt(targetStyles['margin-left' as any], 10),
            targetY: targetRect.top - parseInt(targetStyles['margin-top' as any], 10),
            targetHeight: targetRect.height,
            targetWidth: targetRect.width,
            initialX: this.clientX,
            initialY: this.clientY
            });
    }
    this.isMoved = true;
